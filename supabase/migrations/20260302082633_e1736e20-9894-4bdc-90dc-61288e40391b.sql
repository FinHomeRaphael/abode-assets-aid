...
BEGIN
  ...
  -- Get user info for new household name
  SELECT COALESCE(last_name, first_name) INTO user_name FROM public.profiles WHERE id = _user_id;
  
  -- Get current household currency
  SELECT default_currency INTO user_currency FROM public.households WHERE id = _household_id;
