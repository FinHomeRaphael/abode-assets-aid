...
const MonthlyReportModal = ({ open, onClose }: Props) => {
  ...
  const incomeByCategory = useMemo(() => {
    ...
    transactions.filter(t => t.type === 'income' && t.category !== 'Transfert').forEach(t => {
            ...
            <div>
              ...
              <div className="bg-secondary/20 rounded-xl divide-y divide-border/30 overflow-hidden">
                ...
                <FlowRow
                  ...
                  accounts={accounts}
                  transactions={transactions.filter(t => t.type === 'income' && t.category !== 'Transfert')}
                  formatAmount={formatAmount}
                />
                {/* Dépenses */}
                <FlowRow
                    ...
                    return (
                      <div key={d.id} className="bg-secondary/30 rounded-xl p-3">
                        <div className="flex items-center justify-between mb-1.5">
                          ...
                          <div className="flex items-center gap-2">
                            {ltv !== null && (
                              <span className="font-mono-amount text-[10px] text-muted-foreground">LTV {ltv}%</span>
                            )}
